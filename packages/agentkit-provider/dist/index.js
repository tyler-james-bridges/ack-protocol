...
__export(index_exports, {
  ...
  AckActionProvider: () => AckActionProvider,
  CompareAgentsSchema: () => CompareAgentsSchema,
  GetAgentFeedbacksSchema: () => GetAgentFeedbacksSchema,
  GetAgentSchema: () => GetAgentSchema,
  GetLeaderboardSchema: () => GetLeaderboardSchema,
  GetReputationSchema: () => GetReputationSchema,
  GetTrustCategoriesSchema: () => GetTrustCategoriesSchema,
  GiveKudosSchema: () => GiveKudosSchema,
  ...
  RegisterAgentSchema: () => RegisterAgentSchema,
  SearchAgentsSchema: () => SearchAgentsSchema,
  TipAgentSchema: () => TipAgentSchema,
  UpdateAgentURISchema: () => UpdateAgentURISchema,
  ackActionProvider: () => ackActionProvider
});
...
var import_agentkit2 = require("@coinbase/agentkit");
var import_agentkit3 = require("@coinbase/agentkit");
var import_viem = require("viem");

// src/schemas.ts
var import_zod = require("zod");
var SearchAgentsSchema = import_zod.z.object({
  query: import_zod.z.string().describe("Search query for agent names or descriptions"),
  chainId: import_zod.z.number().optional().describe("Chain ID to filter results (2741=Abstract, 8453=Base, 1=Ethereum). Omit for all chains."),
...
var GetAgentSchema = import_zod.z.object({
  chainId: import_zod.z.number().describe("Chain ID where the agent is registered (e.g. 2741 for Abstract)"),
  tokenId: import_zod.z.number().describe("Agent token ID on the Identity Registry")
}).describe("Get detailed information about a specific ERC-8004 agent");
var GetReputationSchema = import_zod.z.object({
  chainId: import_zod.z.number().describe("Chain ID where the agent is registered"),
  tokenId: import_zod.z.number().describe("Agent token ID")
}).describe("Get reputation breakdown for an ERC-8004 agent");
var GetAgentFeedbacksSchema = import_zod.z.object({
  chainId: import_zod.z.number().describe("Chain ID where the agent is registered"),
  tokenId: import_zod.z.number().describe("Agent token ID"),
  limit: import_zod.z.number().optional().default(20).describe("Maximum number of feedbacks to return")
}).describe("Get feedback/kudos list for an ERC-8004 agent");
var GiveKudosSchema = import_zod.z.object({
  chainId: import_zod.z.number().describe("Chain ID of the target agent (must match wallet network)"),
  agentId: import_zod.z.number().describe("Token ID of the agent to give kudos to"),
  value: import_zod.z.number().min(0).max(100).describe("Kudos score from 0-100 (stored with 0 decimals)"),
  tag1: import_zod.z.string().optional().default("").describe("Primary category tag: reliability, speed, accuracy, creativity, collaboration, security, starred"),
  tag2: import_zod.z.string().optional().default("").describe("Secondary tag (freeform, e.g. 'great response time')")
}).describe("Give onchain kudos/feedback to an ERC-8004 agent via the Reputation Registry");
var RegisterAgentSchema = import_zod.z.object({
  agentURI: import_zod.z.string().describe("Agent metadata URI (IPFS, HTTPS, or data: URI) containing agent name, description, and services")
}).describe("Register a new ERC-8004 agent on the Identity Registry");
var UpdateAgentURISchema = import_zod.z.object({
  agentId: import_zod.z.number().describe("Agent token ID to update"),
  newURI: import_zod.z.string().describe("New agent metadata URI (IPFS, HTTPS, or data: URI)")
}).describe("Update the metadata URI of an existing ERC-8004 agent");
var TipAgentSchema = import_zod.z.object({
  chainId: import_zod.z.number().describe("Chain ID of the agent to tip (e.g. 2741 for Abstract)"),
  agentId: import_zod.z.number().describe("Token ID of the agent to tip"),
  amount: import_zod.z.number().min(0.01).max(100).describe("Tip amount in USDC (min 0.01, max 100)"),
  message: import_zod.z.string().optional().describe("Optional message to include with the tip")
}).describe("Give x402 tipped kudos to an agent (real USDC payment backing the endorsement)");
var GetTrustCategoriesSchema = import_zod.z.object({}).describe("Get the 6 ERC-8004 trust categories with descriptions");
var GetLeaderboardSchema = import_zod.z.object({
  chainId: import_zod.z.number().optional().default(2741).describe("Chain ID to get leaderboard for (default 2741 for Abstract)"),
  limit: import_zod.z.number().optional().default(10).describe("Number of top agents to return (default 10)")
}).describe("Get the top ERC-8004 agents by star count on a given chain");
var CompareAgentsSchema = import_zod.z.object({
  chainIdA: import_zod.z.number().describe("Chain ID of the first agent"),
  tokenIdA: import_zod.z.number().describe("Token ID of the first agent"),
  chainIdB: import_zod.z.number().describe("Chain ID of the second agent"),
  tokenIdB: import_zod.z.number().describe("Token ID of the second agent")
...
var _AckActionProvider = class _AckActionProvider extends import_agentkit.ActionProvider {
  constructor() {
    super("ack", []);
    /**
...
_ts_decorate([
  (0, import_agentkit3.CreateAction)({
    name: "search_agents",
    description: `
...
`,
  ...
  _ts_metadata("design:paramtypes", [
    typeof import_agentkit2.EvmWalletProvider === "undefined" ? Object : import_agentkit2.EvmWalletProvider,
    typeof import_zod2.z === "undefined" || typeof import_zod2.z.infer === "undefined" ? Object : import_zod2.z.infer
  ]),
