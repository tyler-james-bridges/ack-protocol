#!/usr/bin/env npx tsx
/**
 * Register the ACK Assembly intelligence tool on the ERC-8257 ToolRegistry (Base).
 *
 * Pattern follows scripts/register-tool.ts (reputation) and Morsel Tool #28.
 *
 * Usage:
 *   npx tsx scripts/register-assembly-tool.ts --dry-run
 *   PRIVATE_KEY=0x... npx tsx scripts/register-assembly-tool.ts
 *   PRIVATE_KEY=0x... RPC_URL=https://mainnet.base.org npx tsx scripts/register-assembly-tool.ts
 *
 * Flags:
...
function printChecklist(manifest: ReturnType<typeof buildAckAssemblyManifest>) {
  console.log('');
...
async function main() {
  ...
  const localManifest = buildAckAssemblyManifest();
  ...
  if (!useLocal && !dryRun) {
    ...
    console.log('[ok] Using local lib/tools/assembly-manifest.ts');
  }
  ...
  const summary = manifest as ReturnType<typeof buildAckAssemblyManifest>;
  console.log('');
  ...
  console.log(
    `  Price:        $${ASSEMBLY_TOOL_PRICE_USDC} USDC (Base + Abstract x402)`
  );
  console.log(`  Network:      ${networkName}`);
  console.log(
    '  Predicate:    none (open access; x402 advertised in manifest and enforced at POST /api/tool/assembly)'
  );
  console.log('');
  ...
  console.log('Registering assembly tool onchain...');
