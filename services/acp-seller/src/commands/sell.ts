...
function validateOfferingJson(filePath: string): ValidationResult {
  ...
  if (json.subscriptionTiers !== undefined) {
    if (!Array.isArray(json.subscriptionTiers)) {
      result.valid = false;
      result.errors.push(
        'offering.json: "subscriptionTiers" must be an array of tier objects'
      );
    } else {
      ...
      for (const tier of parsedOffering.subscriptionTiers) {
        ...
        if (existing) {
          ...
          if (
            ...
            if (existing.duration !== durationSeconds)
              updates.duration = tier.duration;
            ...
            if (updateResult.success) {
              ...
              allErrors.push(
                `Failed to update subscription tier "${tier.name}"`
              );
            }
          } else {
    ...
    for (const o of offerings) {
      ...
      if (o.subscriptionTiers?.length) {
        output.field(
          ...
          o.subscriptionTiers
            .map(
              (t: InlineSubscriptionTier) =>
                `${t.name} (${t.price} USDC, ${t.duration}d)`
            )
            .join(', ')
        );
      }
